...
function HoverAnimatedLink({ href, children }) {
  return (
    <a href={href} className="relative group text-white text-sm font-medium">
      ...
      {/* Hover underline from center outwards */}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="hidden sm:flex space-x-6">
              {leftLinks.map((link) => (
                <HoverAnimatedLink key={link.name} href={link.href}>
                  {link.name}
                </HoverAnimatedLink>
              ))}
            </div>
            ...
            <div className="relative group inline-block text-white text-2xl font-semibold tracking-wide">
              {/* Center underline animation (same as the links below) */}
              <HoverAnimatedLink href="#shubham-kukreti">
                Shubham Kukreti
              </HoverAnimatedLink>
            </div>
